...
func TestDeterministicRoutingEvaluation(t *testing.T) {
	tests := []struct {
		...
		{name: "list facets", prompt: "Перечисли фасеты", intent: entities.IntentListEntities, source: entities.SourceDomain, expectedTypes: []string{"facets"}},
		{name: "list facet documents", prompt: "Какие документы фасета у нас есть?", intent: entities.IntentListEntities, source: entities.SourceDomain, expectedTypes: []string{"facet-documents"}},
